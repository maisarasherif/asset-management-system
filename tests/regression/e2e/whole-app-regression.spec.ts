...
async function createRegressionFixture(request: APIRequestContext): Promise<RegressionFixture> {
  ...
  const mainCategory = await postJson<CreatedMainCategory>(request, token, "/main-category", {
    main_category_name: mainCategoryName,
    description: "Created by Playwright for whole-app route regression.",
    sort_order: sortOrderBase,
  });

  const category = await postJson<CreatedCategory>(request, token, "/category", {
  ...
  const defaultScope = await getJson<CatalogScope>(request, token, "/catalog-scopes/default");
  ...
  const testType = await postJson<CreatedTestType>(request, token, "/test-type", {
    test_name: testName,
    validity_duration: 12,
    description: "Created by Playwright for whole-app route regression.",
  });

  const template = await postJson<CreatedTemplate>(request, token, "/template", {
    template_name: templateName,
    description: "Created by Playwright for whole-app route regression.",
  });

  await putJson(request, token, `/template/${template.template_id}/configuration`, {
    components: [
      {
...
async function cleanupRegressionFixture(
  ...
  await deleteIfPresent(request, fixture.token, `/asset/${fixture.assetId}`);
  await deleteIfPresent(request, fixture.token, `/template/${fixture.templateId}`);
  await deleteIfPresent(request, fixture.token, `/catalog-scope-category/${fixture.scopeCategoryId}`);
  await deleteIfPresent(request, fixture.token, `/category/${fixture.categoryId}`);
  await deleteIfPresent(
    request,
    fixture.token,
    `/catalog-scope-main-category/${fixture.scopeMainCategoryId}`
  );
  await deleteIfPresent(request, fixture.token, `/main-category/${fixture.mainCategoryId}`);
  await deleteIfPresent(request, fixture.token, `/test-type/${fixture.testId}`);
  await deleteIfPresent(request, fixture.token, `/user-project-access/${fixture.accessId}`);
  await deleteIfPresent(request, fixture.token, `/user/${fixture.clientUserId}`);
}
...
test.describe("whole app regression", () => {
  ...
  test("admin can open every major routed screen against current app data", async ({ page }) => {
    ...
    try {
      ...
      await expectRoute(page, "/dashboard", [fixture.assetName, "Certificate status"]);
      await expectRoute(page, "/assets", ["Assets directory", fixture.assetName]);
      await expectRoute(page, "/assets/new", ["Create asset", "Asset information"]);
      await expectRoute(page, `/assets/${fixture.assetId}/edit`, ["Edit asset", "Asset information"]);
      await expectRoute(page, `/assets/${fixture.assetId}/routine-maintenance`, [
        "Routine maintenance",
        fixture.assetName,
      ]);
      ...
      await expectRoute(page, "/templates", ["Templates", fixture.templateName]);
      await expectRoute(page, "/templates/new", ["Create template", "Template details"]);
      ...
      await expectRoute(page, "/catalog", ["Catalog", fixture.certificateName]);
      await expectRoute(page, "/administration", ["Administration", "Users"]);
      await expectRoute(page, "/client-access", ["Client access", fixture.projectName]);
      await expectRoute(page, "/scheduler", ["Scheduler management", "Notification audit"]);
  ...
  test("client role is confined to the client portal and account routes", async ({ page }) => {
    const setupRequest = await playwrightRequest.newContext();
    let fixture: RegressionFixture | null = null;
    ...
    try {
      ...
      await expectRoute(page, "/client/assets", ["Client asset portal", fixture.assetName]);
      ...
      await expect(page).toHaveURL(/\/client\/assets$/);
      await expect(page.getByText(fixture.assetName, { exact: true }).first()).toBeVisible();
    } finally {
      await cleanupRegressionFixture(setupRequest, fixture);
      await setupRequest.dispose();
    }
